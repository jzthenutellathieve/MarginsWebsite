...
const { shortTitle, slug, displayDate } = require('./editorial.cjs');
function renderArticle(article, snippets, assets) {
  ...
  const projectLinks = isDevice ? '<div class="md-project-links"><a class="md-nav-link" href="/projects/water/">Read the full water project →</a><a href="/projects/water/#laboratory-video">Watch the laboratory video ↓</a></div>' : '';
    ...
    return '<header class="mj-reader-head"><p class="md-kicker"><a href="/articles/topic/'+slug(article.tag)+'/">'+escapeText(article.tag)+'</a> · '+escapeText(article.format || 'Article')+'</p><h1>'+escapeText(shortTitle(article))+'</h1>'+(article.deck ? '<p class="mj-reader-subhead">'+escapeText(article.deck)+'</p>' : '')+'<p class="mj-reader-deck">'+escapeText(article.subtitle)+'</p><div class="mj-reader-meta">'+escapeText(byline)+' · '+escapeText(displayDate(article))+' · '+escapeText(article.readTime)+' read'+(article.award ? '<br>'+escapeText(article.award) : '')+'</div>'+updated+projectNote+draftHistoryNote+reportingNote+toolbar+projectLinks+collaboration+'</header><div class="mj-reader-body">'+diagram+blocks+correction+'<section class="mj-sources" id="article-sources"><h2>Sources</h2><ol>'+sources+'</ol></section></div>';
}
